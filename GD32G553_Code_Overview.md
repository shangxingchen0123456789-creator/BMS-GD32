...
板级配置集中在 `Driver/bms_board_config.h`，其它驱动不再散落硬件比例或引脚映射。
...
`Driver/app_tasks.c` 是系统调度核心。初始化顺序为：
...
- 固件：`Comm/bms_protocol.c/.h`
- 固件通信服务：`Comm/bms_comm_service.c`
...
| `Driver/app_tasks.c` | FreeRTOS 任务编排、首帧真实采样、周期控制 |
| `Driver/bms_board_config.h` | 主控板/功率板硬件映射和标定参数 |
| `Driver/bms_types.h` | 全局枚举、状态结构、故障位 |
| `Driver/bms_state.c` | 控制任务与通信任务之间的状态快照 |
| `Driver/afe_gd30bm2016.c` | BM2016 SPI、保护配置、电芯电压、FET、均衡 |
| `Driver/adc_manager.c` | 功率板 ADC 真实采样和物理量换算 |
| `Driver/safety_manager.c` | 最高优先级故障锁存和快速关断 |
| `Driver/power_path_manager.c` | Q4/Q5 电池路径策略 |
| `Driver/charge_manager.c` | 充电状态机、命令处理、故障汇总 |
| `Driver/power_control.c` | HRTIMER PWM、Buck/Boost 映射、慢速 PI |
| `Driver/balance_manager.c` | 被动均衡位图计算 |
| `Driver/soc_estimator.c` | 基于真实电压和真实电流的 SOC 估算 |
| `Comm/bms_protocol.c` | 二进制协议编解码和 CRC |
| `Comm/bms_comm_service.c` | 串口帧收发调度、周期主动上报 |
| `Comm/bms_uart.c` | USART2 非阻塞 RX/TX 中断环形缓冲 |
