...
bms_command_reply_t Charge_Manager_Handle_Fet_Mask_Command(uint8_t fet_mask)
{
    ...
    taskENTER_CRITICAL();
    if(g_charge_manager.control.workMode != (uint8_t)BMS_WORK_MODE_BMS) {
        taskEXIT_CRITICAL();
        return Reply_Error((uint8_t)BMS_CMD_ERROR_INVALID_MODE);
    }
    blocking_faults = g_charge_manager.fault.latchedFaults | Safety_Manager_Get_Latched_Faults();
    ...
    g_charge_manager.digital.enabled = 0U;
    Charge_Manager_Clear_Path_Settle_Unlocked();
    g_charge_manager.control.runRequest = 0U;
    g_charge_manager.control.state = BMS_CHARGE_STATE_IDLE;
    if(g_charge_manager.control.mode == (uint8_t)BMS_CHARGE_MODE_DIGITAL_POWER) {
     ...
     */
    g_charge_manager.manualFet.active = 1U;
    g_charge_manager.manualFet.mask = fet_mask;
    taskEXIT_CRITICAL();
...
void Charge_Manager_Init(void)
{
    ...
    if(Param_Storage_Get_Charge(&stored_params) != 0U) {
        ...
        if(Charge_Manager_Validate_Parameters(&stored_params) != 0U) {
            g_charge_manager.config.params = stored_params;
        } else {
            Charge_Manager_Default_Parameters(&g_charge_manager.config.params);
            Charge_Manager_Clamp_Parameters(&g_charge_manager.config.params);
        }
    } else {
        Charge_Manager_Default_Parameters(&g_charge_manager.config.params);
        Charge_Manager_Clamp_Parameters(&g_charge_manager.config.params);
    ...
    g_charge_manager.control.runRequest = 0U;
    g_charge_manager.control.mode = (uint8_t)BMS_CHARGE_MODE_AUTO;
    g_charge_manager.control.workMode = (uint8_t)BMS_WORK_MODE_BMS;
    g_charge_manager.fault.latchedFaults = 0U;
    g_charge_manager.fault.presentFaults = 0U;
    g_charge_manager.control.cvDoneCounter = 0U;
    g_charge_manager.digital.enabled = 0U;
    g_charge_manager.digital.targetVoltageMv = 0U;
    g_charge_manager.digital.currentLimitMa = 0U;
    Charge_Manager_Clear_Manual_Fet();
    Charge_Manager_Clear_Path_Settle_Unlocked();
    Charge_Manager_Reset_Output_Ovp_Counters();
    taskEXIT_CRITICAL();
...
void Charge_Manager_Get_Parameters(bms_charge_parameters_t *parameters)
{
    ...
    taskENTER_CRITICAL();
    *parameters = g_charge_manager.config.params;
    taskEXIT_CRITICAL();
}
...
uint8_t Charge_Manager_Is_Digital_Power_Active(void)
{
    ...
    taskENTER_CRITICAL();
    active = ((g_charge_manager.control.workMode == (uint8_t)BMS_WORK_MODE_DIGITAL_POWER) ||
              (g_charge_manager.digital.enabled != 0U) ||
              (g_charge_manager.control.state == BMS_CHARGE_STATE_DIGITAL_POWER) ||
              (g_charge_manager.control.mode == (uint8_t)BMS_CHARGE_MODE_DIGITAL_POWER)) ? 1U : 0U;
    taskEXIT_CRITICAL();
...
bms_command_reply_t Charge_Manager_Set_Work_Mode(uint8_t work_mode)
{
    ...
    taskENTER_CRITICAL();
    g_charge_manager.control.workMode = work_mode;
    g_charge_manager.digital.enabled = 0U;
    Charge_Manager_Clear_Manual_Fet();
    Charge_Manager_Clear_Path_Settle_Unlocked();
    g_charge_manager.control.runRequest = 0U;
    g_charge_manager.control.state = BMS_CHARGE_STATE_IDLE;
    g_charge_manager.control.mode = (work_mode == (uint8_t)BMS_WORK_MODE_DIGITAL_POWER) ?
...
bms_command_reply_t Charge_Manager_Set_Parameters(const bms_charge_parameters_t *parameters)
{
    ...
    g_charge_manager.config.params = clamped_parameters;
    taskEXIT_CRITICAL();
...
bms_command_reply_t Charge_Manager_Handle_Command(uint8_t command_id, uint8_t argument, uint8_t has_argument)
{
    ...
    case CMD_START_CHARGE:
        ...
        if(g_charge_manager.control.workMode != (uint8_t)BMS_WORK_MODE_BMS) {
        ...
        } else if((g_charge_manager.fault.latchedFaults | Safety_Manager_Get_Latched_Faults()) != 0U) {
            ...
            g_charge_manager.control.runRequest = 1U;
            g_charge_manager.control.state = BMS_CHARGE_STATE_PRECHECK;
            g_charge_manager.control.cvDoneCounter = 0U;
            Charge_Manager_Reset_Output_Ovp_Counters();
            reply = Reply_Ok();
        }
    ...
    case CMD_STOP_CHARGE:
        ...
        Charge_Manager_Clear_Manual_Fet();
        Charge_Manager_Clear_Path_Settle_Unlocked();
        if((g_charge_manager.control.workMode != (uint8_t)BMS_WORK_MODE_DIGITAL_POWER) &&
    ...
    case CMD_EMERGENCY_STOP:
        ...
        Charge_Manager_Clear_Path_Settle_Unlocked();
        g_charge_manager.control.runRequest = 0U;
        g_charge_manager.control.state = BMS_CHARGE_STATE_FAULT;
        g_charge_manager.fault.latchedFaults |= BMS_FAULT_EMERGENCY_STOP;
        Charge_Manager_Reset_Output_Ovp_Counters();
        taskEXIT_CRITICAL();
    ...
    case CMD_CLEAR_FAULT:
        (void)Afe_Gd30bm2016_Recover_Protections();
        taskENTER_CRITICAL();
        present_faults = g_charge_manager.fault.presentFaults;
        taskEXIT_CRITICAL();
    ...
    case CMD_SWITCH_MODE:
        if(has_argument == 0U || argument > (uint8_t)BMS_CHARGE_MODE_CV) {
            ...
            Charge_Manager_Clear_Manual_Fet();
            Charge_Manager_Clear_Path_Settle_Unlocked();
            g_charge_manager.control.mode = argument;
            run_request_active = g_charge_manager.control.runRequest;
            if(g_charge_manager.control.runRequest != 0U) {
                /*
                                                               ...
                                                               uint16_t current_limit_ma)
{
    ...
    if(g_charge_manager.digital.enabled != 0U) {
        /*
         * 宸插湪鏁板瓧鐢垫簮杩愯涓紝鐩存帴鏇存柊鐩爣鐢靛帇/鐢垫祦銆?
         * 鐢靛帇鐜細骞虫粦璺熻釜鍒版柊璁惧畾鍊硷紝涓嶅啀鍋滄満娉勬斁鍐嶉噸鍚€?
         * 璋冧綆鐢靛帇鏃剁敱 OVP 杞檺骞呭厹搴曪紝鑰屼笉鏄厛鎺夌數銆?
         */
        g_charge_manager.digital.targetVoltageMv = target_voltage_mv;
        g_charge_manager.digital.currentLimitMa = current_limit_ma;
        Charge_Manager_Clear_Manual_Fet();
        Charge_Manager_Clear_Path_Settle_Unlocked();
        g_charge_manager.control.state = BMS_CHARGE_STATE_DIGITAL_POWER;
        g_charge_manager.control.mode = (uint8_t)BMS_CHARGE_MODE_DIGITAL_POWER;
    ...
    raw_faults = g_charge_manager.fault.latchedFaults | Safety_Manager_Get_Latched_Faults();
    taskEXIT_CRITICAL();
    ...
    taskENTER_CRITICAL();
    g_charge_manager.digital.enabled = 1U;
    g_charge_manager.digital.targetVoltageMv = target_voltage_mv;
    g_charge_manager.digital.currentLimitMa = current_limit_ma;
    Charge_Manager_Clear_Manual_Fet();
    Charge_Manager_Clear_Path_Settle_Unlocked();
